...
class Input extends Component {
  ...
  renderLabel() {
    const {label, labelStyle, multiline, tips, required,size} = this.props;
      ...
      <TouchableWithoutFeedback onPress={this.onPressLabel}>
        <View style={{alignItems:'center',paddingLeft:10,paddingRight:10}}>
          ...
          <Text style={[Style.label, labelStyle]}>
            {required && <Text style={[Style.required]}>*&nbsp;</Text>}<FontAwesome color="#3AA1FE" name={label} size={size}/>
          </Text>
  ...
  render() {
    const {editable,multiline,error,wrapperStyle,focusStyle,
      disabledStyle,errorStyle,initJudge,readOnly,secureTextEntry,
      label,tips} = this.props;
    const {isFocus} = this.state;
    const isColumnMode = multiline || !!tips || (label ? label.length > 12 : false);
