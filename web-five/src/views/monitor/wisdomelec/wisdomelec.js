...
import { getWarnMsg } from '@/util/warn'
import {
    getObj
} from "@/api/unit/floor";
import {
...
export default {
    ...
    data() {
        return {
            ...
            systemTypeList: [], // 系统类型数组
            buildList: [], // 建筑数组
            deviceStatusList: [], // 建筑数组
            tableLoading: false,
            timeout:false,
            timeout1:false,
            degreeOptions: [],
    ...
    methods: {
        ...
        getWaring(id) {
            getWaringTree({
                ...
                if (res && res.data) {
                    for (var ins in res.data.data.filter(s=>s.value==8)) {
                        res.data.data.filter(s=>s.value==8)[ins].disabled = true;
                    }
                    this.equipmentData = res.data.data.filter(s=>s.value==8);
        ...
        // 获取统计图数据
        getEchartdata() {
            analysisList(8).then(res => {
                //   console.log(res)
                this.echartsdatay = res.data.data
            })
            // 获取最近一天数据
            analysisListone({systemType:8}).then(res => {
                // console.log(res)
            ...
            getEquipment(id).then(res => {
                if (res && res.data) {
                    ...
                    if(dataList){
                        ...
                        this.tabOneData.useStatus = dataList.useStatus//设备状态
                        this.tabOneData.deviceName = dataList.deviceName//设备名称
                        this.tabOneData.deviceCode = dataList.deviceCode//设备标识码
                        this.tabOneData.deviceType = this.getDicValue(this.deviceTypeList, dataList.deviceType) //设备类型
                        this.getBuildFloorValue(dataList.buildingId, dataList.floorId)
                        ...
                        this.tabOneData.warnState = this.getDicValue(this.deviceStatusList, dataList.warnState) //设备使用状态
                        this.tabOneData.modelPhoto = dataList.modelPhoto //设备型号图
                        this.tabOneData.surroundingPhoto = dataList.surroundingPhoto //设备现场图
                    }
                }
            })
        },
        ...
        // 弹窗数据匹配建筑楼层
        getBuildFloorValue(buildingId, floorId) {
            let item = this.buildList.building
            let buildHas = false
            for (let i = 0, len = item.length; i < len; i++) {
                if (item[i].id == buildingId) {
                    buildHas = true
                    this.tabOneData.buildingId = item[i].buildingCode //所在建筑
                    this.getFloorAllTree(buildingId).then((res) => {
                        let floorHas = false
                        for (let j = 0, length = res.length; j < length; j++) {
                            if (res[j].id == floorId) {
                                floorHas = true
                                this.tabOneData.floorId = res[j].floorCode //设备楼层
                            }
                        }
                        if (!floorHas) {
                            this.tabOneData.floorId = '楼层信息错误'
                        }
                    })
                }
            }
            if (!buildHas) {
                this.tabOneData.buildingId = '建筑信息错误'
            }
        },
        //相关记录
        getUnitRecord(id) {
        ...
        getCurrentStatus(id) {
            getMotesystem(id).then(res => {
                ...
                if (stutasData) {
                    ...
                    console.log('this.deviceParams', this.deviceParams)
        ...
        //动态曲线查询
        lineSearch() {
            let createTimeStart = this.lineStartData;
            let createTimeEnd = this.lineEndData;
            this.getDynamic({
                deviceId: this.setData.row.deviceId,
        ...
        // 去除h5中 &nbsp;
        filterIndex() {
            let tab1 = document.getElementById('tab-0')
            tab1.innerHTML = tab1.innerHTML.replace(/<i><\/i>&nbsp;/ig, "")
            let tab2 = document.getElementById('tab-1')
