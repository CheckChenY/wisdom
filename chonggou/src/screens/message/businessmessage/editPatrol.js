...
class AddPatrol extends Component {
    static navigationOptions = ({ navigation, navigationOptions }) => {
        return {
            headerTitle: (
                <Text style={{ flex: 1, textAlign: 'center', color: '#fff', fontSize: 18 }}>编辑</Text>
            ),
            headerRight: (
                <Text />
            ),
                        ...
                        <View style={{ flexDirection: 'row', marginTop: 15 }}>
                            <View style={{ width: 90 }}>
                                <View style={{ flexDirection: 'row', justifyContent: 'flex-end' }}>
                                    <Text style={{ color: '#FD3E3C' }}>* </Text><Text style={{ textAlign: 'right', lineHeight: 20, color: '#333', fontSize: 14 }}>巡查卡编码:</Text>
                                </View>
                            </View>
    ...
    submitPop = () => {
        this.popUp.showPop('请完善巡查卡信息')
    }
    submit=()=>{
        const { patrolPointName,cardCode,buildingId,floorId,location,patrolPointType,patrolTaskId,id,list } = this.state;
        if (this.state.cardCode.length !== 14) {
            this.popUp.showPop('卡号长度为14位......')
            return
        }

        // if ( patrolPointName === '' || patrolPointType === '' || cardCode === '' || buildingId === '' || floorId === '' || location === '') {
        ...
        let params={
            ...
            // "patrolTaskId": patrolTaskId,//巡查任务id
            id:id
        }
        console.log(params)
        FetchManager.post('/safe/inner/safePoint/update',params, (set) => {
            console.log(set)
            if(set && set.success){
