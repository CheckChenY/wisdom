...
class AddDev extends Component {
    static navigationOptions = ({ navigation, navigationOptions }) => {
        return {
            headerTitle: (
                <Text style={{ flex: 1, textAlign: 'center', color: '#fff', fontSize: 17 }}>手动添加</Text>
            ),
            headerRight: (
                <TouchableOpacity 
                    ...
                    <View style={{ paddingLeft: 15, paddingRight: 15 }}>
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 15 }}>
                            <View style={{ width: '30%' }}>
                                ...
                                {/* <View>
                                    <Text style={{ textAlign: 'right', color: '#333', fontSize: 14 }}>(设备ID)</Text>
                                </View> */}
                            </View>
    ...
    changeDeviceId = () => {
        if(this.state.code){
            this.getDeviceInfo(this.state.code)
        }
        // this.getDeviceInfo(86822104039812)
        
    }
    // 获取设备信息
    getDeviceInfo = (id) => {
        FetchManager.get('/device/inner/jtlDevice/findByDeviceId?deviceId=' + id,'', async (set) => {
            //下面的就是请求来的数据
            console.log(set)
            if(set && !set.success){
            ...
            if( set && set.success){
                ...
                //     return 
                // }
                // let defaultSystem = SystemDic.filter(item => item.value == data.systemId)
                // let defaultType = initTypeDic.filter(item => item.value == data.deviceType)
                
                this.setState({
                    code:data.deviceId,
                    defaultType: data.type,//设备类型
                    data:data,//主键ID
                })
            }
        })
