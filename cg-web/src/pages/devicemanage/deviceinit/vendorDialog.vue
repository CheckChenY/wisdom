<template>
  <div>
    <a-drawer
		...
		:width="720"
        >
		<a-form class="form_dialog" :form="form" layout="vertical" hideRequiredMark @submit="onSumbit">
			<a-row :gutter="16" v-for="(column,i) in columns" :key="i">
				<a-col :span="24" v-if="column.title">
					<h4 class="title_vendor">{{column.title}}</h4>
				</a-col>
				<a-col :span="12" 
					v-show="!item.display"
					v-for="(item,index) in column.children" :key="index">
