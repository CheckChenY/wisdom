...
import { columns } from '@/const/deviceinit/alarmdict.js';
import { findLicense } from "@/api/public"
import { findPageList, insert, update, delDict } from "@/api/devicemanage/deviceinit/alarmdict"
...
export default {
    ...
    methods: {
        ...
        submit(params, handle) {
            params.alarmDicCode = parseInt(params.alarmDicCode, 16)
            if (params.alarmDicCodeTen) {
                params.alarmDicCode = params.alarmDicCodeTen
            }
            ...
            findPageList(this.params).then(res => {
                if (res && res.data && res.data.success) {
                    ...
                    res.data.value.content.forEach(item => {
                        item.alarmDicCodeTen = item.alarmDicCode
                        item.alarmDicCode = item.alarmDicCode?parseInt(item.alarmDicCode).toString(16):''
