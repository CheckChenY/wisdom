...
import { columns } from '@/const/deviceinit/orderdict.js';
import { findLicense } from "@/api/public"
import { findPageList, insert, update, delDict } from "@/api/devicemanage/deviceinit/orderdict"
...
export default {
    ...
    methods: {
        ...
        submit(params, handle) {
            params.orderDicCode = parseInt(params.orderDicCode, 16)
            if (params.orderDicCodeTen) {
                params.orderDicCode = params.orderDicCodeTen
            }
            ...
            findPageList(this.params).then(res => {
                if (res && res.data && res.data.success) {
                    ...
                    res.data.value.content.forEach(item => {
                        item.orderDicCodeTen = item.orderDicCode
                        item.orderDicCode = item.orderDicCode?parseInt(item.orderDicCode).toString(16):''
