...
import { columns } from '@/const/deviceinit/datadict.js';
import { findLicense } from "@/api/public"
import { findPageList, insert, update, delDict } from "@/api/devicemanage/deviceinit/datadict"
...
export default {
    ...
    methods: {
        ...
        submit(params, handle) {
            params.dataDicCode = parseInt(params.dataDicCode, 16)
            if (params.dataDicCodeTen) {
                params.dataDicCode = params.dataDicCodeTen
            }
            ...
            findPageList(this.params).then(res => {
                if (res && res.data && res.data.success) {
                    ...
                    res.data.value.content.forEach(item => {
                        item.dataDicCodeTen = item.dataDicCode
                        item.dataDicCode = item.dataDicCode?parseInt(item.dataDicCode).toString(16):''
