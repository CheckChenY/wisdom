...
import { columns } from '@/const/deviceinit/threshdict.js';
import { findLicense } from "@/api/public"
import { findPageList, insert, update, delDict } from "@/api/devicemanage/deviceinit/threshdict"
...
export default {
    ...
    methods: {
        ...
        submit(params, handle) {
            params.thresholdDicCode = parseInt(params.thresholdDicCode, 16)
            if (params.thresholdDicCodeTen) {
                params.thresholdDicCode = params.thresholdDicCodeTen
            }
            ...
            findPageList(this.params).then(res => {
                if (res && res.data && res.data.success) {
                    ...
                    res.data.value.content.forEach(item => {
                        item.thresholdDicCodeTen = item.thresholdDicCode
                        item.thresholdDicCode = item.thresholdDicCode?parseInt(item.thresholdDicCode).toString(16):''
