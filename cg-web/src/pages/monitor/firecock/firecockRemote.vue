                ...
                <a-form-item label="最近操作队列">
                    <a-table bordered 
                        ...
                        <span slot="operationResult" slot-scope="operationResult">
                            <a-tag
                                :color="operationResult === '操作成功' || operationResult === '成功' ? '#87d068' : 
                                        operationResult === '正在执行' ? '#108ee9' :
                                        operationResult === '失败' ? '#f50' : ''"
                                :key="operationResult"
                            >   
                                
                                {{ operationResult === '操作成功' || operationResult === '成功' ? '操作成功' : 
                                operationResult === '失败' ? '操作失败' : 
                                operationResult === '正在执行' ? '正在执行' : '' }}

                                <a-icon :type="operationResult === '操作成功' || operationResult === '成功' ? 'check-circle' : 
    ...
    methods: {
        ...
        perTimeInterval () {
            this.perInterval = setInterval(() => {
                ...
                if(this.percent==12 || this.percent > 12){
                    clearInterval(this.perInterval)
                }
            },3000)
        },
