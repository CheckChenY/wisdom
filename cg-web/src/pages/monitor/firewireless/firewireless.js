...
import firewirelessDialog from './firewirelessDialog.vue';
import firewirelessRemote from './firewirelessRemote.vue';
...
export default {
    ...
    methods:{
        getDictData(){
            getDeviceFindList({type:2,id:'1574920051754848'}).then(res=>{
                this.deviceTypeList=[]
        ...
        onView(record) {
            this.$refs.firewirelessDialog.showDrawer(record)
        },
        handleSearch(e) {
            e.preventDefault();
        ...
        closeModal(record, value) {
            if (value == 'Remote'){
                this.$refs.firewirelessRemote.showDrawer(record)
            }
        },
        rowClassName (record) {
            let className = '';
